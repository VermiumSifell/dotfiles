...
local function create_battery_widget()
    local battery_textbox = wibox.widget {widget = wibox.widget.textbox}

    local function update_battery_name(battery)


        local battery_info = {}
        local capacities = {}
    
        for s in battery:gmatch("[^\r\n]+") do
        ...
        battery_textbox.text = get_battery_icon(charge, status) .. " " ..
    ...
    local container_battery_widget = {
        {
            {widget = battery_textbox},
	...
	-- Would be cool to set colors, for example maroon based on percentage
